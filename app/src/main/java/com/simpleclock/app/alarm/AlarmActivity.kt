...
class AlarmActivity : ComponentActivity() {
    ...
    private val occurrenceTokenState = mutableStateOf("")
    ...
    private val dismissReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action == AlarmRingingService.ACTION_DISMISS_ACTIVITY) {
                val dismissedToken = intent.getStringExtra(AlarmScheduler.EXTRA_OCCURRENCE_TOKEN)
                if (dismissedToken == occurrenceTokenState.value) finishAndRemoveTask()
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        occurrenceTokenState.value =
            intent.getStringExtra(AlarmScheduler.EXTRA_OCCURRENCE_TOKEN).orEmpty()
        configureLockScreenWindow()

        setContent {
            val alarmId = alarmIdState.longValue
            val occurrenceToken = occurrenceTokenState.value
            val defaultLabel = stringResource(R.string.alarm_name_default)
            var display by remember(alarmId, occurrenceToken) {
                mutableStateOf(
                    AlarmDisplay(
            ...
            LaunchedEffect(alarmId, occurrenceToken) {
        ...
        try {
            startService(
                Intent(this, AlarmRingingService::class.java)
                    ...
                    .putExtra(
                        AlarmScheduler.EXTRA_OCCURRENCE_KIND,
                        intent.getIntExtra(AlarmScheduler.EXTRA_OCCURRENCE_KIND, -1),
                    )
                    .putExtra(
                        AlarmScheduler.EXTRA_TRIGGER_AT,
                        intent.getLongExtra(AlarmScheduler.EXTRA_TRIGGER_AT, -1L),
                    )
                    .putExtra(
                        AlarmScheduler.EXTRA_OCCURRENCE_TOKEN,
                        occurrenceTokenState.value,
                    ),
            )
