            ...
            try {
                ...
                val mode = when (intent.action) {
                    Intent.ACTION_TIME_CHANGED, Intent.ACTION_TIMEZONE_CHANGED ->
                        AlarmRescheduleMode.RECALCULATE_REGULAR
                    Intent.ACTION_BOOT_COMPLETED, Intent.ACTION_MY_PACKAGE_REPLACED ->
                        AlarmRescheduleMode.RESTORE_AFTER_RESTART
                    else -> AlarmRescheduleMode.RESTORE_EPOCH
                }
                app.database.alarmDao().getEnabled().forEach { alarm ->
                    try {
                        app.alarmScheduler.reschedulePersisted(alarm, mode)
